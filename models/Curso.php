...
class Curso extends Conectar {
    ...
    public function insert_curso($cat_id, $cur_nom, $cur_descrip, $cur_fechini, $cur_fechfin, $inst_id) {
        ...
        parent::set_names();
        $sql = "INSERT INTO tm_curso (cur_id, cat_id, cur_nom, cur_descrip, cur_fechini, cur_fechfin, inst_id, cur_img, fech_crea, est) VALUES (NULL, ?, ?, ?, ?, ?, ?, ?, now(), '1');";
    ...
    public function delete_curso_usuario($curd_id) {
        $conectar = parent::conexion();
        parent::set_names();
        $sql = "DELETE FROM td_curso_usuario WHERE curd_id = ?";
        $sql = $conectar->prepare($sql);
